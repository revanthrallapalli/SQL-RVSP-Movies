...
SELECT Count(title) AS movies_1_count
...
WITH language_movie_votes
     AS (SELECT CASE
         ...
         GROUP  BY language)
SELECT language,
       Sum(movies_votes) AS total_votes
...
AS
  ...
  ON         d.name_id = n.id
  INNER JOIN top_three_genres AS t
  USING      (genre)
  INNER JOIN ratings AS r
...
SELECT name 																						AS actor_name,
       ...
       DENSE_RANK() OVER (ORDER BY Round(Sum(avg_rating * total_votes) / Sum(total_votes), 3) DESC) AS actor_rank
FROM   role_mapping AS rm
       INNER JOIN movie AS m
               ON rm.movie_id = m.id
       INNER JOIN ratings AS r
